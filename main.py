...
df[['Age', 'Sleep Duration', 'Physical Activity Level']] = scaler.fit_transform(df[['Age', 'Sleep Duration', 'Physical Activity Level']])

# Min-Max Normalizasyonu
min_max_scaler = MinMaxScaler()
df[['Stress Level', 'Heart Rate']] = min_max_scaler.fit_transform(df[['Stress Level', 'Heart Rate']])
...
# 30 yaş üstü ve 6 saatten az uyuyanları seç
...
# Yeni bir sahte veri seti oluşturalım
df2 = pd.DataFrame({
    'Person ID': df.index,
...
# Cinsiyet sütununu düzelterek 0 ve 1 olarak kodlayalım (One-Hot Encoding)
